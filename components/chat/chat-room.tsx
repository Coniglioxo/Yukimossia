...
import { AlertCircle, Blocks, Check, Trash2, User, ChevronLeft, ChevronRight, Clapperboard, Clock, Gift, Languages, Loader2, MoreHorizontal, Sparkles, X, Smile } from "lucide-react";
        ...
        <div className={`chat-input-bar chat-room-main-pane flex flex-col ${uiMode === "wechat" ? "chat-input-bar-wechat" : ""}`} data-ui="input" data-chat-ui={uiMode}>
            ...
            <div className="chat-input-actions">
                ...
                <button onClick={onTogglePlusMenu} disabled={inputLocked} className="ui-bare-btn text-[var(--c-text)] chat-action-plus" style={inputLocked ? { opacity: 0.35 } : undefined}>
                    <svg className="chat-action-plus-icon" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="16" /><line x1="8" y1="12" x2="16" y2="12" /></svg>
                </button>
                <button
                    onClick={uiMode === "wechat" && !inputText.trim() ? () => {
                        if (!inputLocked) onTriggerAIResponse();
                        onClosePanels();
                    } : handleSubmit}
                    disabled={uiMode === "wechat" ? (!isGenerating && inputLocked) : (!isGenerating && (inputLocked || !inputText.trim()))}
                ...
                >
                    ...
                    ) : uiMode === "wechat" && !inputText.trim() ? (
                        <Sparkles className="chat-action-reply-icon" size={20} strokeWidth={1.5} aria-hidden="true" />
                    ) : (
            ...
            {showPlusMenu && (
                <div className="chat-plus-menu chat-plus-panel">
                    {plusMenuItems.map((item, i) => (
                        <div
                            key={`${item.label}-${i}`}
                            onClick={item.onClick}
                            className="chat-plus-menu-item chat-plus-panel-item flex flex-col items-center gap-1.5 cursor-pointer"
                            data-chat-plus-item={item.label}
                            {...(item.active ? { "data-active": "" } : {})}
                        >
                            <div className="chat-plus-icon-box chat-plus-panel-icon-box">
                                <span className="chat-plus-panel-icon" aria-hidden="true">{item.icon}</span>
                            </div>
                            <span className="chat-plus-panel-label ts-11 text-[var(--c-text)]">{item.label}</span>
                        </div>
                    ))}
                </div>
            )}
